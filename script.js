...
function trimData(pageNo){
  ...
  return pageData.slice(start,start+row)
}

// creating rows for given json

function createTable(page){
  let trimmedData = trimData(page)
  tbody.innerHTML = '';
  for(let data of trimmedData){
    let tr = document.createElement('tr')
    tr.innerHTML = `<td class="pr-4">${data.id}</td>
                    <td>${data.name}</td>
                    <td>${data.email}</td>`
    tbody.append(tr);
  }
...
function pageNoclicked(event) 
{
  ...
  if(event.target.id=='previous')
  {
    if(parseInt(previousPageNo)-1<1)
      createTable(parseInt(previousPageNo));
    else
    {
      createTable(parseInt(previousPageNo)-1);
      previousPageNo--;
    }
  ...
  else if(event.target.id=='next')
  {
    ...
    previousPageNo = Math.ceil(noOfData/row);
    createTable(Math.ceil(noOfData/row));
    changeActiveClr(event)
...
function changeActiveClr(event)
{
  if(event.target.parentElement.id=='previous-li' || event.target.parentElement.id=='next-li')
  {
    ...
    else if(event.target.parentElement.id=='first-li' || event.target.parentElement.id=='last-li')
    {
      id = previousPageNo+'-li'
      document.getElementById(id).classList.add('active');
    if(previousPageId!=id)
